  ...
  return (
    <div style={{ background: '#FAF8F5' }}>
      ...
      <section style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 0, alignItems: 'center', paddingTop: 80 }} className="max-w-none overflow-hidden relative md:grid-cols-[1fr_1fr]">
        ...
        {/* Left content */}
        <div style={{ padding: 'clamp(40px, 8vw, 80px)', position: 'relative', zIndex: 1 }}>
          <div
      ...
      {/* BRAND MARQUEE */}
      <section style={{ padding: 'clamp(40px, 8vw, 60px) 0', background: '#F0EBE3', overflow: 'hidden' }}>
        <div className="mb-8 text-center">
          <div className="section-label mb-3">Our Brands</div>
          <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 44px)', color: '#1C1916' }}>
        ...
        <div style={{ overflow: 'hidden', position: 'relative' }}>
          <div className="flex animate-marquee" style={{ width: 'max-content', gap: 0 }}>
            {[...brands, ...brands].map((b, i) => (
              <div key={i} className="flex items-center gap-8" style={{ padding: 'clamp(16px, 3vw, 20px) clamp(32px, 5vw, 48px)', borderRight: '1px solid rgba(28,25,22,0.1)', whiteSpace: 'nowrap' }}>
                <span className="font-display" style={{ fontSize: 'clamp(16px, 2.5vw, 22px)', color: '#1C1916', opacity: 0.7 }}>{b.name}</span>
      ...
      {/* EDITORIAL BANNER */}
      <section style={{ position: 'relative', height: 'clamp(300px, 80vw, 600px)', overflow: 'hidden' }}>
        <img
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: 'clamp(20px, 5vw, 80px)' }}>
          <div style={{ maxWidth: 520 }}>
            <div className="section-label mb-4" style={{ color: '#C4A882' }}>Your Ritual Awaits</div>
            <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 66px)', color: '#FAF8F5', marginBottom: 20, lineHeight: 1.05 }}>
              A Better Routine Starts Here.
            </h2>
            <p style={{ fontSize: 'clamp(14px, 2vw, 16px)', color: 'rgba(250,248,245,0.75)', marginBottom: 36, lineHeight: 1.7 }}>
              Build a routine that fits your skin, your lifestyle, and your everyday goals.
            </p>
            <button
              onClick={() => setPage('shop')}
              style={{ background: '#C4A882', color: '#1C1916', padding: '12px 28px', fontSize: 'clamp(11px, 1.5vw, 12px)', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 0.3s ease' }}
            >
              Discover Skincare
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </button>
      ...
      {/* SKINCARE DISCOVERY */}
      <section style={{ padding: 'clamp(60px, 12vw, 100px) 0', background: '#FAF8F5' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <div className="section-label mb-3">Personalise</div>
            <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 52px)', color: '#1C1916', marginBottom: 12 }}>
              Find What Your Skin Needs.
            </h2>
            <p style={{ fontSize: 'clamp(14px, 2vw, 15px)', color: '#8A7E72' }}>Select a concern to discover products curated for you.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 'clamp(16px, 3vw, 20px)' }}>
      ...
      {/* NEW ARRIVALS */}
      <section style={{ padding: 'clamp(60px, 12vw, 80px) 0', background: '#F5F2ED' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-10">
            <div>
              <div className="section-label mb-3">Just Landed</div>
              <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 50px)', color: '#1C1916' }}>
                Fresh Into Your Routine.
              </h2>
            </div>
            <button className="btn-outline" onClick={() => setPage('shop')}>See All New</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 'clamp(16px, 3vw, 20px)' }}>
            {newArrivals.map(p => (
              <ProductCard
      ...
      {/* SPECIAL OFFER */}
      <section style={{ background: '#1C1916', padding: 'clamp(40px, 10vw, 80px) 0' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(30px, 8vw, 60px)', alignItems: 'center' }}>
            <div>
              ...
              <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 58px)', color: '#FAF8F5', marginBottom: 20, lineHeight: 1.05 }}>
                Your Beauty Ritual,
                <span style={{ color: '#C4A882', fontStyle: 'italic' }}> Better.</span>
              </h2>
              <p style={{ fontSize: 'clamp(14px, 2vw, 15px)', color: 'rgba(250,248,245,0.6)', lineHeight: 1.7, marginBottom: 36, maxWidth: 400 }}>
                Discover selected beauty essentials at exclusive prices. Limited-time offers curated just for you.
              </p>
              <button
                onClick={() => setPage('shop')}
                style={{ background: '#C4A882', color: '#1C1916', padding: '12px 28px', fontSize: 'clamp(11px, 1.5vw, 12px)', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10 }}
              >
                Shop Offers
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </button>
      ...
      {/* BEAUTY JOURNAL */}
      <section style={{ padding: 'clamp(60px, 12vw, 100px) 0' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-12">
            <div>
              <div className="section-label mb-3">Journal</div>
              <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 52px)', color: '#1C1916' }}>
                The NAZ Beauty Journal
              </h2>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 'clamp(20px, 4vw, 24px)' }}>
            {journalPosts.map(post => (
              <div key={post.id} className="group cursor-pointer card-hover" style={{ background: '#FFFFFF', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  <h3 className="font-display" style={{ fontSize: 'clamp(18px, 2.5vw, 20px)', color: '#1C1916', lineHeight: 1.3, marginBottom: 12 }}>{post.title}</h3>
                  <p style={{ fontSize: 'clamp(12px, 1.5vw, 13px)', color: '#8A7E72', lineHeight: 1.6, marginBottom: 16 }}>{post.excerpt}</p>
...
function NewsletterSection() {
  ...
  return (
    <section style={{ background: '#F0EBE3', padding: 'clamp(60px, 12vw, 100px) 0' }}>
      <div className="max-w-2xl mx-auto px-6 text-center">
        <div className="section-label mb-5">Newsletter</div>
        <h2 className="font-display" style={{ fontSize: 'clamp(28px, 4vw, 56px)', color: '#1C1916', marginBottom: 16 }}>
          Stay In The Glow.
        </h2>
        <p style={{ fontSize: 'clamp(14px, 2vw, 15px)', color: '#8A7E72', lineHeight: 1.7, marginBottom: 48, maxWidth: 400, margin: '0 auto 48px' }}>
        ...
        {submitted ? (
          <div className="animate-fade-up" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <div className="font-display" style={{ fontSize: 'clamp(20px, 4vw, 24px)', color: '#1C1916' }}>You're in the glow.</div>
            <p style={{ fontSize: 'clamp(13px, 1.5vw, 14px)', color: '#8A7E72' }}>Welcome to the NAZ community.</p>
          </div>
        ) : (
          <form
            onSubmit={e => { e.preventDefault(); if (email) setSubmitted(true) }}
            className="flex gap-3 max-w-sm mx-auto flex-col md:flex-row"
          >
            <input
              type="email"
              value={email}
